...
const ProfileScreen = ({ history }) => {
  ...
  const consultationApproval = useSelector(
    (state) => state.consultationApproval
  );
  ...
  }, [dispatch, userInfo, loadingApproval, refresh]);
            ...
            <Table striped borderless responsive>
              <tbody>
                ...
                <tr>
                  ...
                  <td>{userInfo.specialization}</td>
                </tr>
                <tr>
                  <td>
                    <strong>Affiliation</strong>
                  </td>
                  <td>{userInfo.affiliation}</td>
                </tr>
              </tbody>
            </Table>
          )}
                ...
                <tbody>
                  {patients.map((patient, index) => (
                    <tr key={index}>
                      ...
                      <td>{patient.email}</td>
                      <td>
                        <LinkContainer to={`/patient/${patient.id}`}>
                ...
                {consultations.map((consul) => (
                  <Alert variant='info' className='p-2' key={consul._id.$oid}>
                    <Row className='align-items-center'>
                      <Col md={10}>
                        <p className='m-0'>
                          <strong>{`${consul.patientName}`}</strong> has
                          requested your consultation.{' '}
                          <Link
